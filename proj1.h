...
typedef nx_struct {
  ...
  nx_uint16_t   period_count;// The number of periods that have occured.
  nx_uint16_t   x_cnt;       //
...
typedef nx_struct {
  ...
  nx_uint16_t   ts;          // the time at which the original packet was created
  nx_uint8_t    buff[L_MSG_BUFFSIZE]; // the message buffer
} lmsg_t;
...
typedef nx_struct {
  ...
  nx_uint8_t    len;               // the length of the queue, for our example this will be = Q_LEN
  nx_uint8_t    size;              // the current size of the Q
