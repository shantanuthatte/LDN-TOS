...
implementation {
  ...
  bool loadQ( msg_q_t *q, lmsg_t *m ) {
    ...
    if(q->size >= Q_LEN){                 // Q is full
      return( q_loaded );
    }else{
      if(tailIndex >= Q_LEN){             //Tail reached end, rearrange Q
      ...
      memcpy((lmsg_t *)&q->pkt[tailIndex], m, sizeof(lmsg_t));
...
printf( "SEND: Type:%-2x To:%-2d Src: %-2d Dst: %-2d Seq: %-4d\n",
  ...
  void init() {
   int i;
    ...
    gState.query_period = gState.period / QUERY_INT;
