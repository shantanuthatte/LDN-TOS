...
implementation {
  ...
  bool sendWait( ) {
    ...
    while (gState.send_busy) {          // wait for the send to complete
      ...
      if (cnt >= N_SEND_WAIT) {
	printf( "error: sendWait loop at %d\n", N_SEND_WAIT );
	printfflush( );
	return( FALSE );
      }
    }
  ...
  bool rcvWait( ) {
    ...
    while (gState.send_busy) {          // wait for the send to complete
      ...
      if (cnt >= N_SEND_WAIT) {
	printf( "error: RcvsendWait loop at %d\n", N_SEND_WAIT );
	printfflush( );
	return( FALSE );
      }
    }
  ...
  */ 
  ...
  void processQuery_cmd( msg_q_t *q, lmsg_t *m ) {
    ...
    if (gSnd_busy) {
      printf( "error: process query cmd, send is busy\n" );
      printfflush( );
      return;
    }
  ...
  event void Boot.booted() {
    ...
    gState.send_busy = FALSE;
    gState.rcv_busy = FALSE;
  ...
  event message_t* Receive.receive(message_t *msg, void *payload, uint8_t len) {
    ...
    if ((gRcv_busy) || (gSnd_busy)) {
      printf( "error: in rcv but Rcv is %d and Snd is %d\n", gRcv_busy, gSnd_busy );
      printfflush( );
      return( msg );
    }
  ...
  event void QTimer.fired( ) {
    ...
    if (gSnd_busy) {
      printf( "error: process qTimer fired, send is busy\n" );
      printfflush( );
      return;
    }
  ...
  event void AMSend.sendDone(message_t* msg, error_t error) {
    gSnd_busy = FALSE;           // the mote is no longer send busy, send is completed
