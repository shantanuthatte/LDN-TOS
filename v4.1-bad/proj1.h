...
enum {
  ...
  PKT_RATE_RANGE = 400,      // The range of packet generation per period
  MIN_PKT_RATE = 40,         // The minimum packet generation rate per period
  QUERY_INT = 200,           // query time interval, the time between neighboring node
  ...
  PROC_INT = 10,              // Interval at which to process pending recd packets.
  SND_INT = 50,               // Interval at which to send pending packets.
};
...
typedef nx_struct {
  ...
  nx_uint8_t    link[N_NODES+1];  //Indicates a valid link
  nx_uint8_t    active[N_NODES+1];//Indicates an active link
  nx_bool       activeT;
  nx_bool       PActiveT;
  nx_am_addr_t  pktDst;
} state_t;
...
typedef nx_struct {
  ...
  nx_uint16_t   load_ts[Q_LEN+1];    // the time that the msg at index i was loaded into the Q
                                   // their is not a message in the Q at this index
  lmsg_t        pkt[Q_LEN+1];

  nx_uint16_t   headIndex;
  nx_uint16_t   tailIndex;
} msg_q_t;
